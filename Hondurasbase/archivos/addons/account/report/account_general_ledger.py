...
class general_ledger(report_sxw.rml_parse, common_report_header):
    ...
    def set_context(self, objects, data, ids, report_type=None):
        if data['form']['filtrar_cuenta']:      
            data['form']['ids']=data['form']['cuentas']
            new_ids = data['form']['cuentas']
        else:
            new_ids = ids
    ...
    def lines(self, account):
        obj_periodo_fiscal=self.pool.get('account.period').search(self.cr,self.uid,[('fiscalyear_id','=',self.context['fiscalyear']),('special','=',True)])
        """ Return all the account_move_line of account with their account code counterparts """
        move_state = ['draft','posted']
        if self.target_move == 'posted':
            move_state = ['posted', '']
        ...
        """ %(self.query, tuple(move_state),obj_periodo_fiscal[0],sql_sort)
        self.cr.execute(sql, (account.id,))
        res_lines = self.cr.dictfetchall()
        res_init = []
        if res_lines and self.init_balance:
