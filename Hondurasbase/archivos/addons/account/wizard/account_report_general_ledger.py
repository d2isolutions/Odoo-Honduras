...
class account_report_general_ledger(osv.osv_memory):
    ...
    def check_report3(self, cr, uid, ids, context=None):
        ...
        data['form'] = self.read(cr, uid, ids, ['date_from',  'date_to',  'fiscalyear_id','filtrar_cuenta','cuentas', 'journal_ids', 'period_from', 'period_to',  'filter',  'chart_account_id', 'target_move'], context=context)[0]
        ...
        if data['form']['filtrar_cuenta']:
            data['ids']=data['form']['cuentas']
        ...
        for a in data['ids']:
            ...
            if len(self.get_children_accounts(cr, uid, ids,account,data['form']['fiscalyear_id'],context))>1:
                row += 2
            res_company=account.company_id.name
            if not self.amount_currency2:
                for account2 in self.get_children_accounts(cr, uid, ids,account,data['form']['fiscalyear_id'],context):
                    scredit=self._sum_credit_account(cr, uid, ids,account2,data['form']['fiscalyear_id'],context)
                    sdebit=self._sum_debit_account(cr, uid, ids,account2,data['form']['fiscalyear_id'],context)
                    sprogres=self._sum_balance_account(cr, uid, ids,account2,data['form']['fiscalyear_id'],context)
                    debit_total+=sdebit
                    ...
                    row+=1
                    array.append(res)
                    for line in  self.lines(cr, uid, ids,account2,data['form']['fiscalyear_id'],context):
                        res = {
                            'ldate': line['ldate'],
                            'move': line['move'],
                ...
                for account2 in self.get_children_accounts(cr, uid, ids,account,data['form']['fiscalyear_id'],context):
                    scredit=self._sum_credit_account_currency(cr, uid, ids,account2,data['form']['fiscalyear_id'],context)
                    sdebit=self._sum_debit_account_currency(cr, uid, ids,account2,data['form']['fiscalyear_id'],context)
                    sprogres=self._sum_balance_account_currency(cr, uid, ids,account2,data['form']['fiscalyear_id'],context)
                    debit_total+=sdebit
                    credit_total+=scredit
                    progres_total+=sprogres
                    res = {
                    ...
                    for line in  self.lines2(cr, uid, ids,account2,context):
                        ...
                        row+=1
                        array.append(res)
            if len(self.get_children_accounts(cr, uid, ids,account,data['form']['fiscalyear_id'],context))>1 or len(data['ids'])>1 :
    ...
    def _sum_credit_account_currency(self,cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        a=obj_periodo_fiscal[0]
    ...
    def _sum_debit_account_currency(self,cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        a=obj_periodo_fiscal[0]
        if account.type == 'view':
    ...
    def _get_sortby(self,data):
        if self.sortby2 == 'sort_date':
            ...
            return _('Number')
        return _('Date')
    def _sum_debit_account(self, cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        a=obj_periodo_fiscal[0]
        if account.type == 'view':
            return account.debit
        ...
        '''if self.init_balance2:
            ...
            sum_debit += cr.fetchone()[0] or 0.0'''
        return sum_debit
    def _sum_credit_account(self, cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        a=obj_periodo_fiscal[0]
        if account.type == 'view':
            return account.credit
        ...
        '''if self.init_balance2:
            ...
            sum_credit += cr.fetchone()[0] or 0.0'''
        return sum_credit
    def _sum_balance_account(self, cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        a=obj_periodo_fiscal[0]
        if account.type == 'view':
        ...
        if self.init_balance2:
            ...
            sum_balance += cr.fetchone()[0] or 0.0
        return sum_balance
    def get_children_accounts(self, cr, uid, ids,account,fiscalyear,context):
        res = []
        ...
        for child_account in self.pool.get('account.account').browse(cr, uid, ids_acc, context=context):
            ...
            sold_account = self._sum_balance_account(cr,uid,ids,child_account,fiscalyear,context)
        ...
        if not res:
            return [account]
        return res
    def _sum_balance_account_currency(self, cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        a=obj_periodo_fiscal[0]
        if account.type == 'view':
            return account.balance
    ...
    def lines2(self, cr, uid, ids,account,context):
        ...
        return res
    def lines(self, cr, uid, ids,account,fiscalyear,context):
        obj_periodo_fiscal=self.pool.get('account.period').search(cr,uid,[('fiscalyear_id','=',fiscalyear),('special','=',True)])
        """ Return all the account_move_line of account with their account code counterparts """
        move_state = ['draft','posted']
        if self.target_move2 == 'posted':
        ...
        sql = """
            ...
            JOIN account_journal j on (l.journal_id=j.id)
            WHERE %s AND m.state IN %s AND l.account_id = %%s 
            AND m.period_id<>%s ORDER by %s
        """ %(self.query2, tuple(move_state),obj_periodo_fiscal[0], sql_sort)
        cr.execute(sql, (account.id,))
        res_lines = cr.dictfetchall()
        res_init = []
        if res_lines and self.init_balance2:
    ...
    def _print_report(self, cr, uid, ids, data, context=None):
        ...
        data['form'].update(self.read(cr, uid, ids, ['landscape','filtrar_cuenta','cuentas','initial_balance', 'amount_currency', 'sortby'])[0])
