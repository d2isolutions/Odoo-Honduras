...
class Currency_Historial(osv.osv):
	...
	def init(self,cr):
		...
		cr.execute("""UPDATE res_currency SET active=False        WHERE (name<>'USD') and (name<>'HNL') and (name<>'PEN') and (name<>'GTQ') and (name<>'QTQ');""")

		#Posición del símbolo antes de la cantidad
		cr.execute("""UPDATE res_currency SET position = 'before' WHERE  name ='USD'  or   name ='HNL' or name='PEN' ;""")
